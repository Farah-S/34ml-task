...
class BadgeController extends Controller
{
    public function badges()
    {
        ...
        if(count($currentBadge)>0){
            ...
            $currentBadge=Badge::orderBy('id', 'asc')->first();
            $nextBadge=Badge::where('id', '>', $currentBadge->id)->orderBy('id')->first();
            $remaining=$nextBadge->required_ach - $currentBadge->required_ach;

            return response()->json(['status' => 'success', 'current_badge' => $currentBadge->title ,'next_badge'=>$nextBadge->title, 'remaining_to_unlock_next_badge'=>$remaining]);
